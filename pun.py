...
def guess():
	data = g.get()
	data=data.lower()
	q = """select songname from punsongs where songname=%s"""
	c.execute(q,(data,))
	data2 = c.fetchone()
	randsong = random_mp3.split('.')
	if data2:
		if (randsong[0].lower() in data2[0]):
...
def play():
	global random_mp3
	folder_path = "./punjabi_songs"
	random_file=[]
	mp3_files = [file for file in os.listdir(folder_path) if file.endswith('.mp3')]
	random_mp3= random.choice(mp3_files)
	print(random_file)
	print(random_mp3 not in random_file)
